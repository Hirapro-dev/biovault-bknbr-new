...
async function getPickupPosts(): Promise<Post[]> {
  try {
    const pickups = await prisma.post.findMany({
      where: { published: true, isPickup: true },
...
export default async function HomePage({
  ...
  // ページネーション用basePath
  const paginationBase = catSlug ? `/?cat=${catSlug}` : "/";
            ...
            <div className="pt-4 pb-2 border-b border-black/10 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <div className="flex items-center gap-1 min-w-max">
                ...
                {menuCategories.map((cat) => (
                  <Link key={cat.id} href={`/?cat=${cat.slug}`}
                    className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors whitespace-nowrap ${
          ...
          <div className="lg:flex lg:gap-10">
            <div className="flex-1 min-w-0">
                ...
                <section className="pt-8 pb-6 border-b border-black/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {pickupPosts.map((post: Post) => (
                      <PostCard key={post.id} post={post} variant="pickup" basePath="" />
                    ))}
                  </div>
                </section>
              )}
              <section className="pt-8 pb-6">
                ...
                {data.posts.length > 0 ? (
                  <>
                    <div className="hidden md:block">
                      {data.posts.map((post: Post) => (
                        <PostCard key={post.id} post={post} variant="list" basePath="" />
                      ))}
                    </div>
                    <div className="md:hidden grid grid-cols-2 gap-x-4 gap-y-6 pt-6">
                      {data.posts.map((post: Post) => (
                        <PostCard key={post.id} post={post} variant="grid" basePath="" />
                      ))}
                    </div>
                  ...
                  <div className="text-center py-20">
                    <h2 className="text-lg font-bold text-black">記事はまだありません</h2>
                    <p className="text-black/40 text-sm mt-1">{q ? "検索条件に一致する記事がありません" : activeCategoryName ? `${activeCategoryName}の記事はありません` : "公開中の記事はありません"}</p>
                  </div>
                )}
              </section>
              ...
              <div className="lg:hidden">
                <div className="pt-6 pb-4 border-t border-black/10">
                ...
                {banners.length > 0 && (
                  <section className="pt-6 pb-6 border-t border-black/10">
                ...
                {recommended.length > 0 && (
                  <section className="pt-6 pb-8 border-t border-black/10">
                          ...
                          <Link href={`/${post.slug}`} className="group flex gap-3">
                            <div className="w-28 h-20 shrink-0 rounded-sm overflow-hidden bg-black/5">
                              {post.eyecatch ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center"><span className="font-black text-lg text-black/20">KWR</span></div>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
