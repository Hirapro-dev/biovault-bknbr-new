...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState<StatsData>({
    todayViews: 0, todayClicks: 0, last7DaysViews: 0, last7DaysClicks: 0,
  });
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-lg border-l-4 border-blue-500 border border-slate-200 p-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-lg border border-slate-200">
            <p className="text-slate-400 text-lg mb-4">記事がありません</p>
                  ...
                  {filteredPosts.map((post) => (
                    <tr key={post.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-5 py-3.5">
                        <Link href={`/${post.slug}`} target="_blank" rel="noopener noreferrer" className="font-semibold text-slate-900 text-sm hover:text-blue-600 hover:underline">
                          {post.title}
                        </Link>
                      </td>
                ...
                <div key={post.id} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <Link href={`/${post.slug}`} target="_blank" rel="noopener noreferrer" className="font-semibold text-slate-900 text-sm truncate block hover:text-blue-600 hover:underline">
                        {post.title}
                      </Link>
                      {post.writer && (
                      ...
                      <div className="flex items-center gap-3 mt-2 flex-wrap">
                        ...
                        <span className="text-xs text-slate-400 flex items-center gap-1"><FiTrendingUp size={11} />{post.views.toLocaleString()}</span>
