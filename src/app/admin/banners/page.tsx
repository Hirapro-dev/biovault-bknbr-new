...
export default function BannersPage() {
  ...
  const cancelEdit = () => {
    setEditingId(null);
    setEditLabel(""); setEditUrl(""); setEditImageUrl("");
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await fetch(`/api/banners/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ label: editLabel.trim(), url: editUrl.trim(), imageUrl: editImageUrl.trim() || null }),
      });
      if (res.ok) {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <p className="text-sm text-slate-500 mb-6">
          トップページのサイドバーに、外部リンクのバナーを表示できます。
        </p>
