...
export default function EditPost({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [res, ceRes, wRes, catRes] = await Promise.all([
        fetch(`/api/posts/${id}`),
      ...
      if (res.ok) {
        ...
        setShowForWel(post.showForWel === true);
        ...
        if (post.categories) setSelectedCategoryIds(post.categories.map((pc: { category: { id: number } }) => pc.category.id));
      } else {
        setLoadError(true);
      }
                ...
                <label key={cat.id} className="flex items-center gap-1.5 cursor-pointer">
                  <input type="checkbox" checked={selectedCategoryIds.includes(cat.id)}
                    onChange={(e) => {
                      ...
                      else setSelectedCategoryIds(selectedCategoryIds.filter((cid) => cid !== cat.id));
                    }}
        ...
        <div className="mb-4 md:mb-6">
          ...
          {eyecatch ? (
            <div className="relative rounded-lg overflow-hidden border border-slate-200">
              <div className="aspect-video relative"><Image src={eyecatch} alt="アイキャッチ" fill className="object-cover" sizes="800px" /></div>
